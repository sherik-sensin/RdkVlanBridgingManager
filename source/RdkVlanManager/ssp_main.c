    ...
    switch ( command )
    {
        case    'e' :
...

            {
                ...
                if ( g_Subsystem[0] != 0 )
                {
                    _ansc_sprintf(CName, "%s%s", g_Subsystem, COMPONENT_ID_VLAN_MANAGER);
                }
                else
                {
                    _ansc_sprintf(CName, "%s", COMPONENT_ID_VLAN_MANAGER);
                }
...
int main(int argc, char* argv[])
{
    ...
    {
        CcspTraceInfo(("NonRoot feature is enabled, dropping root privileges for RdkVlanManager Process\n"));
        init_capability();
    ...
    pComponentName          = COMPONENT_NAME_VLAN_MANAGER;
